...
def read_file(folder, name, dtype=None):
    # 使用 osp.join 函数构建目标文件的完整路径，该路径由文件夹路径和文件名（附带 '.txt' 扩展名）组成
    path = osp.join(folder, '{}.txt'.format(name))
    # read_txt_array 从文本文件中读取数组数据， return 返回从文本文件读取的数组数据
    return read_txt_array(path, sep=',', dtype=dtype)

def parse_txt_file_FN(data_path, fpath, line_parse_fn=None):
    with open(data_path + fpath, 'r') as f:
        lines = f.readlines()
    data = [line_parse_fn(s) if line_parse_fn is not None else s for s in lines]
    return data

def read_graph_nodes_relations_FN(data_path, fpath):
    graph_ids = np.load(data_path + fpath).tolist()
...
def read_graph_adj_FN(data_path, fpath, nodes, graphs):
    edges = parse_txt_file_FN(data_path, fpath, line_parse_fn=lambda s: s.split(','))
...
def read_node_features_FN(data_path, fpath, nodes, graphs):
    node_features_all = sp.load_npz(data_path + fpath)#.flatten().tolist()
    node_features_all = node_features_all.todense().tolist()
    node_features = {}
...
def degree_as_node_label_FN(adj_list):
    node_labels = []
    for adj in adj_list:
        deg = [sum(row) for row in adj]
        unique_degrees = sorted(set(deg))
        degree_to_index = {degree: idx for idx, degree in enumerate(unique_degrees)}
        # 将每个节点的度数映射到相应的索引
        nlabels = [degree_to_index[degree] for degree in deg]
        node_labels.append(nlabels)
    return node_labels
...
def create_one_hot(indices, max_value):
    one_hot_matrix = np.zeros((len(indices), max_value + 1))
    one_hot_matrix[np.arange(len(indices)), indices] = 1
    return one_hot_matrix


'''
DataReader类的主要任务是从原始文件中加载数据，执行必要的预处理步骤:从文本文件中解析图结构、特征和标签等，并将这些信息以结构化的形式保存下来
'''
...
class DataReader():
    ...
    def __init__(self, args):
        ...
        if args.dataset == 'politifact' or args.dataset == 'gossipcop':
            ...
            node, graph = read_graph_nodes_relations_FN(self.data_path, '/raw/node_graph_id.npy')
            data['adj_list'] = read_graph_adj_FN(self.data_path, '/raw/A.txt', node, graph)
            data['nlabels'] = degree_as_node_label_FN(data['adj_list'])

            data['labels'] = np.load(self.data_path + '/raw/graph_labels.npy').tolist()
            # 转换成int类型
            data['labels'] = [int(i) for i in data['labels']]
            data['attr'] = read_node_features_FN(self.data_path, f'/raw/new_{args.feature}_feature.npz', node, graph)
            ...
            features = data['attr'] # original features
        ...
        # TODO: 与SBA-ST对比的数据集
        elif args.dataset in ['NCI109', 'MUTAG', 'IMDBBINARY', 'DD', 'COLLAB']:
            self.data_path = os.path.join(args.data_path, args.dataset)
            self.rnd_state = np.random.RandomState(args.seed)
            print('loading data')
            label_dict = {}
            ...
            with open(self.data_path + f'/{args.dataset}.txt', 'r') as f:
                ...
                for i in range(n_g):
                    ...
                    graphs.append((g, node_tags, node_features, label_dict[l]))
                    max_degree = max(max_degree, max(dict(g.degree()).values(), default=0))
                    tag_set.update(node_tags)

            # Create global one-hot encodings based on max_degree and all tags
            global_degree_one_hot = identity(max_degree + 1).toarray()
            global_tag_one_hot = identity(max(tag_set) + 1).toarray()

            for g, node_tags, node_features, label in graphs:
                adj_matrix = nx.adjacency_matrix(g).todense()
                adj_matrix = np.maximum(adj_matrix, adj_matrix.T)  # Making symmetric

                degree_one_hot = global_degree_one_hot[[deg for _, deg in g.degree()]]
                tag_one_hot = global_tag_one_hot[node_tags]

                if node_features:
                    node_features = np.stack(node_features)
                    combined_features = np.concatenate([node_features, degree_one_hot, tag_one_hot], axis=1)
                else:
                    combined_features = np.concatenate([degree_one_hot, tag_one_hot], axis=1)

                data['adj_list'].append(np.array(adj_matrix))
                data['nlabels'].append(node_tags)
                data['labels'].append(label)
                data['features'].append(combined_features)
                data['n_node_max'] = max(data['n_node_max'], len(g))

            data['num_classes'] = len(label_dict)
            data['num_features'] = data['features'][0].shape[1] if data['features'] and data['features'][
            ...
            """
            '''
            lambda f: 声明了一个匿名函数，f是这个函数的参数。
            f.find('graph_indicator') 调用字符串的find方法查找子字符串'graph_indicator'在字符串f中的位置。
            如果找到了，find方法返回子字符串第一次出现的索引（一个非负整数）；如果没有找到，返回-1。
            >= 0 检查find方法的返回值是否大于或等于0，即子字符串是否存在于字符串f中。
            例如，这个表达式可以用在filter函数中，过滤出包含'graph_indicator'的文件名列表
            '''
...
'''
用于从图数据中构建特征矩阵的。
它主要考虑三部分来源的特征：节点标签、节点原始特征（如果有的话）、以及节点的度（Degree）。
这些特征将被合并成每个图实例的节点特征矩阵
'''
...
'''
GraphData对象通常用于将DataReader处理后的数据转换成适合训练图神经网络（GNN）模型的格式
'''
